...
class TestSQSTools:
    ...
    def test_register_sqs_tools_with_disallow_resource_creation(self):
        ...
        with pytest.MonkeyPatch.context() as mp:
            mp.setattr("awslabs.amazon_sns_sqs_mcp_server.sqs.AWSToolGenerator", mock_aws_tool_generator)
    ...
    def test_validator_with_different_operations(self):
        ...
        result, _ = is_mutative_action_allowed(
            mock_mcp, mock_sqs_client, {'QueueUrl': 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue', 'MessageBody': 'Hello world'}
        )
        assert result is True

        # Test with receive_message (uses QueueUrl)
        result, _ = is_mutative_action_allowed(
            mock_mcp, mock_sqs_client, {'QueueUrl': 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue', 'MaxNumberOfMessages': 10}
        )
        assert result is True

        # Test with send_message_batch (uses QueueUrl)
        result, _ = is_mutative_action_allowed(
            mock_mcp, mock_sqs_client, {'QueueUrl': 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue', 'Entries': []}
        )
        assert result is True

        # Test with delete_message (uses QueueUrl)
        result, _ = is_mutative_action_allowed(
            mock_mcp, mock_sqs_client, {'QueueUrl': 'https://sqs.us-east-1.amazonaws.com/123456789012/test-queue', 'ReceiptHandle': 'receipt-handle'}
        )
        assert result is True
    ...
    def test_create_queue_override_implementation(self):
        ...
        # Mock SQS client
        mock_sqs_client = MagicMock()
        mock_sqs_client.create_queue.return_value = {"QueueUrl": "https://sqs.us-east-1.amazonaws.com/123456789012/test-queue"}
        mock_sqs_client_getter = MagicMock(return_value=mock_sqs_client)
        ...
        # Verify result
        assert result == {"QueueUrl": "https://sqs.us-east-1.amazonaws.com/123456789012/test-queue"}
    ...
    def test_create_queue_override_with_fifo_and_custom_attributes(self):
        ...
        # Mock SQS client
        mock_sqs_client = MagicMock()
        mock_sqs_client.create_queue.return_value = {"QueueUrl": "https://sqs.us-east-1.amazonaws.com/123456789012/test-queue.fifo"}
        mock_sqs_client_getter = MagicMock(return_value=mock_sqs_client)
        ...
        # Verify result
        assert result == {"QueueUrl": "https://sqs.us-east-1.amazonaws.com/123456789012/test-queue.fifo"}
